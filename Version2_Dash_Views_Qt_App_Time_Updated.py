...
def describe():
    ...
    tabledata = sys.argv[2]
    dataset = sys.argv[3]
    dataquerry = "SELECT PairPlots FROM" + ' ' +  tabledata + ' ' + " where TableName=" + '"' +dataset + '"' 
    query = QSqlQuery(dataquerry)
    while query.next():
         data  = query.value(0)
         #image = data[0][0]
         ...
         et = time.time()
         est_time = et - st
         console.append("Processing Time is:" + str(est_time) + " " + "Seconds")
    query1 = QtSql.QSqlQuery()
    eventquerry = "SELECT EventTime FROM" + ' ' + tabledata + ' ' + "where TableName=" + '"' +dataset + '"'
    query1 = QSqlQuery(eventquerry)
    while query1.next():
         dateTime = QDateTime()
...
def histogram():
    ...
    tabledata = sys.argv[2]
    dataset = sys.argv[3]
    dataquerry = "SELECT HistogramPlots FROM" + ' ' +  tabledata + ' ' + " where TableName=" + '"' +dataset + '"' 
    query = QSqlQuery(dataquerry)
    while query.next():
         data1  = query.value(0)
         #image = data[0][0]
...
def correlation():
    ...
    tabledata = sys.argv[2]
    dataset = sys.argv[3]
    dataquerry = "SELECT CorrelationPlots FROM" + ' ' +  tabledata + ' ' + " where TableName=" + '"' +dataset + '"' 
    query = QSqlQuery(dataquerry)
    while query.next():
         data2  = query.value(0)
         #image = data[0][0]
...
def missing():
    ...
    tabledata = sys.argv[2]
    dataset = sys.argv[3]
    dataquerry = "SELECT MissingPlots FROM" + ' ' +  tabledata + ' ' + " where TableName=" + '"' +dataset + '"' 
    query = QSqlQuery(dataquerry)
    while query.next():
       data3  = query.value(0)
         #image = data[0][0]
...
def userquerry():
    ...
    def retrievedescribe():
        ...
        tabledata = sys.argv[2]
        dataset = sys.argv[3]
        dataquerry = "SELECT DescribeView FROM" + ' ' +  tabledata + ' ' + " where TableName=" + '"' +dataset + '"' 
        query = QSqlQuery(dataquerry)
    ...
    def retrievecount(): 
        ...
        tabledata = sys.argv[2]
        dataset = sys.argv[3]
        dataquerry = "SELECT CountInfo FROM" + ' ' +  tabledata + ' ' + " where TableName=" + '"' +dataset + '"' 
        query = QSqlQuery(dataquerry)
        while query.next():
            data1  = query.value(0)
            binary_data1 = base64.b64decode(data1)
    ...
    def retrieveinfo():  
        ...
        tabledata = sys.argv[2]
        dataset = sys.argv[3]
        dataquerry = "SELECT InfoView FROM" + ' ' +  tabledata + ' ' + " where TableName=" + '"' +dataset + '"' 
        query = QSqlQuery(dataquerry)
        while query.next():
            data2  = query.value(0)
            binary_data2 = base64.b64decode(data2)
    ...
    def retrievehead():  
        ...
        tabledata = sys.argv[2]
        dataset = sys.argv[3]
        dataquerry = "SELECT HeadInfo FROM" + ' ' +  tabledata + ' ' + " where TableName=" + '"' +dataset + '"' 
        query = QSqlQuery(dataquerry)
        while query.next():
            data3  = query.value(0)
            binary_data3 = base64.b64decode(data3)
...
db.setHostName("172.16.51.38")
#db.setDatabaseName("weather")
db.setDatabaseName(str(dbdata))
db.setUserName("sois")
